...
import vjudge.submitter.UVALiveSubmitter;
...
public class UVALiveSpider extends Spider
{
	...
	public void crawl() throws Exception
	{
		...
		// 抓标题、时限
		HttpGet getMethod = new HttpGet("https://icpcarchive.ecs.baylor.edu/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem="
				+ realProblemNumber);
		...
		String pdfLink = "<span style='float:right'><a target='_blank' href='https://icpcarchive.ecs.baylor.edu/external/"
				+ category
				+ "/"
				+ problem.getOriginProb()
				+ ".pdf'><img width='100' height='26' border='0' title='Download as PDF' alt='Download as PDF' src='https://icpcarchive.ecs.baylor.edu/components/com_onlinejudge/images/button_pdf.png'></a></span><div style='clear:both'></div>";
		...
		try
		{
			...
			entity = response.getEntity();
			html = EntityUtils.toString(entity).replaceAll("(?i)(src|href)\\s*=\\s*(['\"]?)\\s*(?!\\s*['\"]?\\s*http)",
					"$1=$2https://icpcarchive.ecs.baylor.edu/external/" + category + "/");

			// some problems' description are fucking long, only get the
			// body.innerHTML
			html = html.replaceAll("(?i)^[\\s\\S]*<body[^>]*>", "").replaceAll("(?i)</body>[\\s\\S]*", "");

			description.setDescription(pdfLink
					+ Tools.regFind(html, "^([\\s\\S]*?)<H2><FONT size=4 COLOR=#ff0000><A NAME=\"SECTION000100\\d000000000000000\">"));
			description.setInput(Tools.regFind(html,
					"Int?put</A>&nbsp;</FONT>\\s*</H2>([\\s\\S]*?)<H2><FONT size=4 COLOR=#ff0000><A NAME=\"SECTION000100\\d000000000000000\">"));
			description.setOutput(Tools.regFind(html,
					"Output</A>&nbsp;</FONT>\\s*</H2>([\\s\\S]*?)<H2><FONT size=4 COLOR=#ff0000><A NAME=\"SECTION000100\\d000000000000000\">"));
			description.setSampleInput(Tools.regFind(html,
					"Sample Int?put</A>&nbsp;</FONT>\\s*</H2>([\\s\\S]*?)<H2><FONT size=4 COLOR=#ff0000><A NAME=\"SECTION000100\\d000000000000000\">"));
			...
			description.setDescription("<style type=\"text/css\">h1,h2,h3,h4,h5,h6{margin-bottom:0;}div.textBG p{margin: 0 0 0.0001pt;}</style>"
