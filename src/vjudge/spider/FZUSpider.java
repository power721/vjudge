...
public class FZUSpider extends Spider {
	...
	public void crawl() throws Exception{
		...
		try {
			...
			html = Tools.getHtml(getMethod, null).replaceAll("(?i)(src|href)\\s*=\\s*(['\"]?)\\s*(?!\\s*['\"]?\\s*http)", "$1=$2http://acm.fzu.edu.cn/");
		} finally {
		...
		description.setDescription(Tools.regFind(html, "Problem Description</h2></b><div class=\"pro_desc\">([\\s\\S]+?)</div>\\s*<h2><img"));
		...
		description.setSampleInput("<pre>" + Tools.regFind(html, "Sample Input</h2>\\s*<div class=\"data\">([\\s\\S]*?)</div>\\s*<h2><img") + "</pre>");
		description.setSampleOutput("<pre>" + Tools.regFind(html, "Sample Output</h2>\\s*<div class=\"data\">([\\s\\S]*?)</div>\\s*(<h2><img|</div>\\s*<br />)") + "</pre>");
