...
public class UVASpider extends Spider
{
	...
	public void crawl() throws Exception
	{
		...
		// 抓标题、时限
		GetMethod getMethod = new GetMethod("http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem="
				+ realProblemNumber);
		...
		String pdfLink = "<span style='float:right'><a target='_blank' href='http://uva.onlinejudge.org/external/"
				+ category
				+ "/"
				+ problem.getOriginProb()
				+ ".pdf'><img width='100' height='26' border='0' title='Download as PDF' alt='Download as PDF' src='http://uva.onlinejudge.org/components/com_onlinejudge/images/button_pdf.png'></a></span><div style='clear:both'></div>";
		...
		try
		{
			...
			html = Tools.getHtml(getMethod, null).replaceAll("(?i)(src|href)\\s*=\\s*(['\"]?)\\s*(?!\\s*['\"]?\\s*http)",
					"$1=$2http://uva.onlinejudge.org/external/" + category + "/");
