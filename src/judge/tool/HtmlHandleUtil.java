...
public class HtmlHandleUtil
{
	...
	private static final String[][] pairs = new String[][] { { "img", "src" }, { "script", "src" }, { "link", "href" }, { "a", "href" } };
	...
	public static String transformUrlToAbs(String html, String baseUri)
	{
		...
		return doc.toString().replaceAll(">\\s*<", "><").replaceAll("(?<=\\w)\\s+<(?!/pre)", " <").replaceAll("(?<!pre)>\\s+(?=\\w)", "> ");
	}
